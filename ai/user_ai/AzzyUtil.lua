-------------------------------
-- This file is part of AzzyAI 1.50
...
-- Written by Dr. Azzy of iRO Ymir
AUVersion="1.50 dev 17.1"
-------------------------------
...
function GetMobCount(skill,level,target,aggro)
	...
	if skillaoe==nil then
		...
		range=(skillaoe-1)/2
		for k,v in pairs(Targets) do
			dist = GetDistanceAR(target,k)
			if dist <= range and (v[2]>0 or aggro~=0) then
				mobcount=mobcount+1*GetTact(TACT_WEIGHT,k)
			end
		end
	end
...
function GetFASTargetCount(myid,directionx,directiony,aggro)
	...
	if s==1 or s==-1 then -- straight
		...
		for k,v in pairs(Targets) do
			...
			if mx>=xmin and xmax>=mx and my>=ymin and ymax>=my then
				count=count+1
			end
		end
...
function Attack(myid,target)
	if SuperPassive==1 then
		logappend("AAI_ERROR","Attack() called while in SuperPassive. MyEnemy "..MyEnemy.." MyState "..MyState)
	end
...
function AttackRange(myid,skill,level)
	if (skill==nil) then
		logappend("AAI_ERROR","AttackRange called with nil skill. State:"..MyState.." MySkill "..MySkill)
		skill=MySkill
		level=MySkillLevel
	end
	local a     = 0
	if (skill == 0) then
		...
		if (mertype > 10 or mertype == -1) then
			a     = 1
		else
			a     = GetV(V_ATTACKRANGE,myid)
		end
	elseif (skill==MER_CRASH) then
		a 	= 100
	...
	--	a	= 1
		...
		if a==nil then
			a	= GetV (V_SKILLATTACKRANGE,myid,skill)	
		end
	end
	if level == nil then
...
function Closest(myid,ox,oy,range,alt)
	...
	if newx<=0 or newy<=0 then 
		if alt ==nil then
			logappend("AAI_ERROR","Closest() Failure!"..newx.." , "..newy.." alt = none logstring="..logstring)
		else
			logappend("AAI_ERROR","Closest() Failure!"..newx.." , "..newy.." alt ="..alt.." logstring="..logstring)
		end
	end
	return newx,newy
end
...
function GetSkillInfo(skill,info,level)
	if skill~=nil and info~=nil then
		if SkillInfo[skill]~=nil then
			if SkillInfo[skill][info]~=nil then
				if level ==nil then
					return SkillInfo[skill][info]
				elseif SkillInfo[skill][info][level]~=nil then
					...
					if info==2 then
						logappend("AAI_ERROR","GetSkillInfo(): Range for unknown skill requested, returning builtin value"..GetV(V_SKILLATTACKRANGE,MyID,skill).." "..skill)
				...
				logappend("AAI_ERROR","GetSkillInfo(): No skill info type "..info.." found for this skill "..FormatSkill(skill,level))
				if info==2 then
...
function GetCastTime(skill,level)
	
end

function GetDelayTime(skill)
