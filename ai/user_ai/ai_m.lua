...
function WriteStartupLog(Version,ErrorCode,ErrorInfo)
	...
	elseif string.gfind(AUVersion,verspattern)()~="1.50" then
		ErrorCode="File version error"
...
if CVersion==nil then
	...
	elseif string.gfind(CVersion,verspattern)()~="1.50" then
		ErrorCode="File version error"
	...
	elseif string.gfind(MainVersion,verspattern)()~="1.50" then
		ErrorCode="File version error"
	...
	--]]
	if fsize("./AI/USER_AI/M_Config.lua")==2950 then
		ConfigVers="Default"
	else
		ConfigVers="Custom, edited "..LastSavedDate
	end
	if fsize("./AI/USER_AI/M_Tactics.lua")==496 then
		TacticVers="Default"
	else
